...
export class ProfileCommand extends SubCommandPluginCommand {
  public async about(message: Message, args: Args): Promise<Message | void> {
    try {
      ...
      if (!profileDetails) {
        throw new CodeyUserError(message, `${user.username} has not set up their profile!`);
      } else {
  ...
  public async set(message: Message, args: Args): Promise<void> {
    try {
      ...
      if (typeof description === 'boolean') {
        throw new CodeyUserError(message, 'Please enter a description.');
      }
      ...
      if (reason === 'valid' && message.member) {
        ...
        throw new CodeyUserError(message, `${customization} has been set!`);
      }
      ...
      throw new CodeyUserError(message, messagePrefix + reason);
    } catch (e: unknown) {
      if (e instanceof CodeyUserError) {
        e.sendToUser();
      }
