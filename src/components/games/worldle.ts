...
export const gameActionRow = new ActionRowBuilder<ButtonBuilder>().addComponents(hintButton, quitButton);
...
export const findCountryByName = (name: string): Country | null => {
  ...
  // try exact match
  const exactMatch = countriesCache.find(c => 
  ...
  const partialMatch = countriesCache.find(c => 
